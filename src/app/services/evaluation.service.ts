...
export class EvaluationService {
  ...
  getByClassroom(classroomId: string): Observable<EvaluationDto[]> {
    return this.http.get<any>(`${this.apiUrl}/classroom/${classroomId}`).pipe(map(res => res.data ?? res));
  }

  saveGradesBulk(evaluationId: number, body: { grades: Array<{ studentId: number; value?: number; status: string }> }): Observable<any> {
    return this.http.post<any>(`${this.apiUrl}/${evaluationId}/grades`, body).pipe(map(res => res.data ?? res));
  }

  getGrades(evaluationId: number): Observable<any[]> {
    return this.http.get<any>(`${this.apiUrl}/${evaluationId}/grades`).pipe(map(res => res.data ?? res));
  }
}
